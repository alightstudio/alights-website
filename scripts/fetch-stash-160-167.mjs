  ...
  try {
        ...
        try {
          ...
          if (json && (json.data?.list || json.data?.videoList || Array.isArray(json.data))) {
            ...
            if (list && list.length > 0 && list[0]?.id && list[0]?.title) {
              apiData = list;
              console.error(`  API captured: ${list.length} works from ${rUrl.split('/').pop()}`);
            }
          }
        } catch {}
      }
    ...
    if (nextDataText) {
      try {
        ...
        // Search all props for an array of works
          ...
          if (val && typeof val === 'object' && !Array.isArray(val)) {
            for (const k2 of Object.keys(val)) {
              ...
              if (Array.isArray(v2) && v2.length > 0 && v2[0]?.id && v2[0]?.title) {
                works = v2;
                console.error(`  __NEXT_DATA__ found works in props.${key}.${k2}: ${v2.length} items`);
                break;
              }
            }
            if (works) break;
        ...
        if (!works) {
          console.error(`  __NEXT_DATA__ found but no works array. Keys: ${Object.keys(props).join(', ')}`);
        }
      } catch (e) {
    ...
    // Method 2: Use API intercepted data
    if (!works && apiData) {
      works = apiData;
    ...
    if (works && works.length > 0) {
      ...
      writeFileSync(`/tmp/stash${stashId}-debug.html`, html);
      console.error(`  Debug HTML saved to /tmp/stash${stashId}-debug.html`);
    }
  } catch (e) {
